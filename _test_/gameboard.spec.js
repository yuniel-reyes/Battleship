...
test("place ships by calling Ship", () => {
  const playerBoard = Gameboard("player1");
  playerBoard.placeShip("cruiser", 3, "horizontal", [41, 42, 43]);

  expect(playerBoard.shipsToPlaceArr[0].shipType).toBe("cruiser");
});
...
test("test for attack true", () => {
  const playerBoard = Gameboard("player1");
  playerBoard.placeShip("cruiser", 3, "horizontal", [32, 33, 34]);

  // This should return the times_hit+1
  const shipWasHitted = playerBoard.receiveAttack(33);
  expect(shipWasHitted).toBe(true);
});

test("test for attack false", () => {
